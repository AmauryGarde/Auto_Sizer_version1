...
def vinfo_summary(vinfo_db, vpartition_db, vhost_db,
    ...
    if top_ram is not None and top_sto is not None:
        drop_vms_ram = vinfo_db.sort_values('Memory').iloc[:top_ram]['VM']
        temp_sto_df = vpartition_db[['VM', 'Consumed MB']].groupby(['VM'], as_index=False).sum()
        drop_vms_sto = temp_sto_df.sort_values('Consumed MB').iloc[:top_sto]['VM']
        ...
        temp_sto_df = vpartition_db[['VM', 'Consumed MB']].groupby(['VM']).sum()
        drop_vms_sto = temp_sto_df.sort_values('Consumed MB').iloc[:top_sto]['VM']
    ...
    if filter1 is not None and filter2 is not None:
        return output_list, temp_df_sto_vinfo, temp_df_ram_vinfo
    elif filter1 is not None:
        return output_list, temp_df_ram_vinfo
    elif filter2 is not None:
        return output_list, temp_df_sto_vinfo
    else:
        return output_list
...
def get_sizer_info(df_vinfo, df_vpartition, df_vhost, exclude_vm, filter1, filter12, filter2, filter22, sto_unit,
    ...
    if filter1 is not None and filter2 is not None:
        value_dict, df_sto, df_ram = vinfo_summary(df_vinfo, df_vpartition, df_vhost, no_run, pow_off, sto_unit,
                                                   filter1, filter12, filter2, filter22, out_vm, top_ram, top_sto)
        row_3 = html.Div([
        ...
        value_dict, df_ram = vinfo_summary(df_vinfo, df_vpartition, df_vhost, no_run, pow_off, sto_unit,
                                           filter1, filter12, filter2, filter22, out_vm, top_ram, top_sto)
        row_3 = html.Div([
        ...
        value_dict, df_sto = vinfo_summary(df_vinfo, df_vpartition, df_vhost, no_run, pow_off, sto_unit,
                                           filter1, filter12, filter2, filter22, out_vm, top_ram, top_sto)
        row_3 = html.Div([
        ...
        value_dict = vinfo_summary(df_vinfo, df_vpartition, df_vhost, no_run, pow_off, sto_unit, filter1, filter12,
